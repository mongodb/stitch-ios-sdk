...
internal final class StitchAuthImpl: CoreStitchAuth<StitchUserImpl>, StitchAuth {
    ...
    public func providerClient<Provider>(forProvider provider: Provider) -> Provider.Client where Provider : AuthProviderClientSupplier {
        return provider.client(withRequestClient: self.requestClient, withRoutes: self.authRoutes, withDispatcher: self.dispatcher)
    }
    
    public func providerClient<Provider>(forProvider provider: Provider, withName name: String) -> Provider.Client where Provider : NamedAuthProviderClientSupplier {
        return provider.client(forProviderName: name, withRequestClient: self.requestClient, withRoutes: self.authRoutes, withDispatcher: self.dispatcher)
    }
    
    public func login(withCredential credential: StitchCredential, _ completionHandler: @escaping ((StitchUser?, Error?) -> Void)) {
    ...
    internal func link(withCredential credential: StitchCredential, withUser user: StitchUserImpl, _ completionHandler: @escaping ((StitchUser?, Error?) -> Void)) {
        dispatcher.run(withCompletionHandler: completionHandler) {
