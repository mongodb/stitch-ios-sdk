...
public protocol StitchAuth {
    ...
    /**
     * Retrieves the authentication provider client associated with the authentication provider type specified in the argument.
     *
     * - parameters:
     *     - forProvider: The authentication provider conforming to `AuthProviderClientSupplier` which will provide the
     *                    client for this authentication provider. Use the `clientProvider` field of the desired authentication provider class.
     * - returns: an authentication provider client whose type is determined by the `Client` typealias in the type specified
     *            in the `forProvider` parameter.
     */
    func providerClient<Provider: AuthProviderClientSupplier>(forProvider provider: Provider) -> Provider.Client
    
    /**
     * Retrieves the authentication provider client associated with the authentication provider with the specified name and type.
     *
     * - parameters:
     *     - forProvider: The authentication provider conforming to `NamedAuthProviderClientSupplier` which will provide the
     *                    client for this authentication provider. Use the `NamedClientProvider` field of the desired authentication provider class.
     ...
     *     - user: A `StitchUser` object representing the user that the client is now authenticated as, or `nil` if the login failed.
     ...
     * or is logged out.
     *
     * - important: StitchAuthDelegates registered here are stored as `weak` references, meaning that if there are no more
     *              strong references to a provided delegate, its `onAuthEvent(:fromAuth)` method will no longer be
